...
6. If issues found: sends specific `file:line` feedback to the builder and waits for fixes
7. Signs off by writing `.harnest/playtest-report.md` with APPROVED verdict
...
- **Playwright MCP** (optional): Enable in `settings.local.json` for browser-based playtesting. Copy `claude/settings.local.json.example` to `claude/settings.local.json` and set `disabled: false`.
